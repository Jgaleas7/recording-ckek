...
const RENAME_API_URL = 'http://localhost:4002/api/recordings/rename-video'
...
const formatCronPattern = (time, day) => {
  ...
  // Convert numeric day to day name for cron library
  const dayMap = {
    'Sunday': '0',
    'Monday': '1',
    'Tuesday': '2',
  ...
  // The cron library uses different format than node-cron
  // Format: second minute hour dayOfMonth month dayOfWeek
  return `${second} ${minute} ${hour} * * ${dayMap[day]}`;
};
...
const scheduleStartTask = (task, capturer, index) => {
  ...
  // Creating job with cron instead of node-cron
...
const calculateChunks = (startTimeStr, endTimeStr, chunkDuration) => {
  console.log(`Calculando chunks desde ${startTimeStr} hasta ${endTimeStr} con duración ${chunkDuration} minutos`);
...
const scheduleTask = async (taskId) => {
  try {
    ...
    if (task.chunk_time > 0) {
      ...
      const startSchedule = scheduleStartTask(task, capturer, 0);
